...
public:
    ...
    GridVolume(const Properties &props) : Base(props), m_props(props) {
        ...
        const std::string interpolation_name = props.string("interpolation_mode", "trilinear");
        if(interpolation_name == "catmull-approx"){
            m_interpolation_mode = InterpolationMode::CatmullApprox;
        }else if(interpolation_name == "trilinear"){
            m_interpolation_mode = InterpolationMode::Trilinear;
        }else if(interpolation_name == "trilinear-gradient"){
            m_interpolation_mode = InterpolationMode::TrilinearGradient;
        }else{
            Throw("Unknown interpolation mode: ", interpolation_name);
        }
    ...
    std::vector<ref<Object>> expand() const override {
        ...
        switch (m_metadata.channel_count) {
            case 1:
                result = m_raw ? (Object *) new Impl<1, true>(m_props, m_metadata, m_data, m_interpolation_mode)
                               : (Object *) new Impl<1, false>(m_props, m_metadata, m_data, m_interpolation_mode);
                break;
            case 3:
                result = m_raw ? (Object *) new Impl<3, true>(m_props, m_metadata, m_data, m_interpolation_mode)
                               : (Object *) new Impl<3, false>(m_props, m_metadata, m_data, m_interpolation_mode);
                break;
            default:
...
public:
    ...
    GridVolumeImpl(const Properties &props, const VolumeMetadata &meta,
               const DynamicBuffer<Float> &data, const InterpolationMode interpolation_mode)
        : Base(props) {
    ...
    MTS_INLINE auto interpolate(Point3f p, const Wavelength &wavelengths,
        ...
        switch(m_interpolation_mode){
            ...
            case CatmullApprox:{
                ...
                Vector3f sample0( trilinear(voxels, f, rf, -1, 0, 0),
                                  trilinear(voxels, f, rf, 0, -1, 0),
                                  trilinear(voxels, f, rf, 0, 0, -1) );

                Vector3f sample1( trilinear(voxels, f, rf, +1, 0, 0),
                                  trilinear(voxels, f, rf, 0, +1, 0),
                                  trilinear(voxels, f, rf, 0, 0, +1) );

                /*Vector3f scaledPosition = p * max_coordinates - 0.5f;
                f = scaledPosition - enoki::floor(scaledPosition);*/

                Vector3f correctionPolynomial = 0.5f * f * (f - 1.0f);

                sample += dot(sample0 - Vector3f(2.0f * sample) + sample1, correctionPolynomial);
            ...
            case TrilinearGradient: {
                ...
                Float sample = trilinear(vox, f, rf, 0, 0, 0);
                ...
                if constexpr (with_gradient){
                    ...
                    for(int y = 0; y < 2; y++)
                    for(int x = 0; x < 2; x++){
                        grad[z][y][x].x() = - vox[z+1][y+1][x] + vox[z+1][y+1][x+2];
                        grad[z][y][x].y() = - vox[z+1][y][x+1] + vox[z+1][y+2][x+1];
                        grad[z][y][x].z() = - vox[z][y+1][x+1] + vox[z+2][y+1][x+1];
                    }

                    Vector3f normal = trilinear(grad, f, rf, -1, -1, -1);
                    //std::cout << normal << std::endl;
                    return std::make_pair(ResultType(sample), normalize(normal));
                }else{
                    return ResultType(sample);
                }
            }
        }
    }
    ...
    std::string to_string() const override {
        ...
        oss << "GridVolume[" << std::endl
            ...
            << "  interpolation_mode = " << m_interpolation_mode << std::endl
            << "]";
        return oss.str();
    }
...
private:
    ...
    ValueType trilinear(const ValueType (&data)[N][N][N], const Point3f &f, const Point3f &rf,
                         int dx, int dy, int dz) const {

        dx+=1, dy+=1, dz+=1;
        const auto  d000 = data[dz][dy][dx],
                    d001 = data[dz][dy][dx+1],
                    d010 = data[dz][dy+1][dx],
                    d011 = data[dz][dy+1][dx+1],
                    d100 = data[dz+1][dy][dx],
                    d101 = data[dz+1][dy][dx+1],
                    d110 = data[dz+1][dy+1][dx],
                    d111 = data[dz+1][dy+1][dx+1];

        ValueType  v00 = fmadd(d000, rf.x(), d001 * f.x()),
                    v01 = fmadd(d010, rf.x(), d011 * f.x()),
                    v10 = fmadd(d100, rf.x(), d101 * f.x()),
                    v11 = fmadd(d110, rf.x(), d111 * f.x());
        ValueType  v0  = fmadd(v00, rf.y(), v01 * f.y()),
                    v1  = fmadd(v10, rf.y(), v11 * f.y());
        ValueType  result = fmadd(v0, rf.z(), v1 * f.z());
