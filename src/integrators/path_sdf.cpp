...
class PathSdfIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    PathSdfIntegrator(const Properties &props) : Base(props) { }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        masked(si.uv, si.uv.x() > 1000.f) = Vector2f(0.f, 0.f);
    ...
    std::string to_string() const override {
        return tfm::format("PathSdfIntegrator[\n"
            "  max_depth = %i,\n"
            "  rr_depth = %i\n"
...
MTS_IMPLEMENT_CLASS_VARIANT(PathSdfIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(PathSdfIntegrator, "Path Tracer integrator");
